...
# ============ 统一仿真框架 ============
def simulate_all_awgn(snr_db, nblocks=100, block_len=8000):
    ber = {}
    stats = {'Hamming74': [0, 0], 'R3': [0, 0], 'Conv': [0, 0], 'BCH157': [0, 0]}
    ...
    for _ in tqdm(range(nblocks), desc=f"SNR={snr_db}dB"):
        ...
        nbit_err = np.sum(decoded != data_cut_bch)
        stats['BCH157'][0] += nbit_err
...
def plot_all_awgn():
    ...
    ber_dict = {k: [] for k in ['Hamming74', 'R3', 'Conv', 'BCH157']}

    for snr in snr_db_list:
        ber = simulate_all_awgn(snr, nblocks=100, block_len=8000)
        for k in ber_dict:
            ber_dict[k].append(ber[k])
        print(f"SNR={snr}dB:", {k: f"{ber[k]:.3e}" for k in ber})
    ...
    plt.xlabel('SNR (dB)')
    plt.ylabel('BER')
    plt.title('BER Comparison of Different Codes on AWGN') # soft decision
    plt.grid(True, which='both')
    plt.legend()
    plt.tight_layout()
    plt.ylim(1e-9, 1)
    # 保存
    save_figure("ber_awgn_comparison_minus5_35dB.png")
    plt.show()
